...
- Robot execution risk.
  机器人执行风险。
- Sourced facts with a clickable source and publication date, separated from AI inferences and items requiring human validation.
  带可点击来源与发布日期的来源事实，并与 AI 推断、待人工验证项分开展示。
...
- Research target markets with web search and separate sourced facts, AI inferences, and items requiring human validation.
  通过网页搜索研究目标市场，并区分来源事实、AI 推断和待人工验证项。
...
- Use the OpenAI Responses API with `gpt-5.6-terra` by default and validate every task with Structured Outputs.
  默认使用 OpenAI Responses API 与 `gpt-5.6-terra`，并通过 Structured Outputs 校验每类任务。
- The browser calls only the project's Vercel Functions. `OPENAI_API_KEY` must never be included in browser code.
  浏览器只调用项目自身的 Vercel Functions，`OPENAI_API_KEY` 禁止进入浏览器代码。
...
- Localization variants.
  本地化变体。
- Live localization research with source citations.
  带来源引用的联网本地化研究。
- Final Recipe page.
  最终菜谱页。
