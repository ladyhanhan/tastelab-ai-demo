...
Research each target market using live web search. Separate sourced facts from model inferences and items that require customer validation. Include source URLs only when they came from search results. For every sourced fact, include the publication date shown by the source; when no date is available, explicitly return "not provided" in the requested language. Treat costs as assumptions unless the user supplied verified cost data. Explain ingredient substitutions, taste adjustments, and robot-executability impact without claiming legal compliance.
`,
